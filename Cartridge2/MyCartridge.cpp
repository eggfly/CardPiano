...
void Cartridge::tone(double freq, long duration_ms) {
  ...
  while (millis() - start_ms < duration_ms) {
    ...
    sigmaDeltaWrite(p1_channel, volume);
    while (micros() - start_us < half_cycle) {
    }
    start_us = micros();
    sigmaDeltaWrite(p1_channel, 0);
    while (micros() - start_us < half_cycle) {
...
void Cartridge::render_audio() {
  ...
  // Serial.printf("%d,%d,%d,%d\n", p1_output, p2_output, n_output, t_output);
  // Serial.printf("%d\n", p1_output);
  ...
  sigmaDeltaWrite(p1_channel, volume * p1_output * 12);
  sigmaDeltaWrite(p2_channel, volume * p2_output * 12);
  sigmaDeltaWrite(n_channel, volume * n_output * 6);
  sigmaDeltaWrite(t_channel, volume * t_output * 16);
  ...
  //  sum += n_output * 0.7;
  //  sum += t_output;
  //  sigmaDeltaWrite(t_channel, sum * 8);
